...
# --------------------------------------------------------------------------- #
# User guide (docs/GHID.md + PDF, screenshots regenerated by docs/make_guide.py)
# --------------------------------------------------------------------------- #

def test_guide_references_only_existing_screenshots_and_all_of_them():
    import re
    docs = Path(__file__).parent / "docs"
    md = (docs / "GHID.md").read_text(encoding="utf-8")
    referenced = set(re.findall(r"\]\(screenshots/([^)]+)\)", md))
    on_disk = {p.name for p in (docs / "screenshots").glob("*.png")}
    assert referenced and referenced <= on_disk, referenced - on_disk
    assert on_disk <= referenced, on_disk - referenced          # no orphan screenshots
    assert (docs / "GHID.pdf").stat().st_size > 500_000            # images embedded, not broken


def test_guide_is_attached_to_releases_and_linked_from_readme():
    root = Path(__file__).parent
    wf = (root / ".github" / "workflows" / "release.yml").read_text(encoding="utf-8")
    assert "docs/GHID.pdf" in wf and "actions/checkout" in wf.split("release:")[1]
    readme = (root / "README.md").read_text(encoding="utf-8")
    assert "docs/GHID.pdf" in readme and "docs/GHID.md" in readme
